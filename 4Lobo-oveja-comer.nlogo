...
to setup
 ...
 ask patches[ ;; crear el monto inicial de pasto en el ambiente
   set monto-pasto random-float 10.0
   set pcolor scale-color green monto-pasto 0 20
 ]
  create-ovejas n-ovejas [
    setxy random-xcor random-ycor
    set color white
    set shape "ovejas"
    set energia 100
  ]
   create-lobos n-lobos [
    setxy random-xcor random-ycor
    set color brown
    set shape "lobo"
    set energia 100
...
to go
  ask ovejas [
    girar
    mover
    comprobar-si-muere ;; llamar al procedimiento para validad si esta oveja muere por falta de energìa
    comer ;; llama al procedimiento para comer
  ]
  ask lobos [
    girar
    mover
...
to mover
  forward 1
  set energia energia - costo-movimiento;; Actualizar la energía de la oveja o lobo
end

to comer
 if(monto-pasto >= energia-del-pasto)[ ;; comprobar que el pasto tenga la energia suficiente para ser proporcionada a la oveja
   set energia energia + energia-del-pasto ;; actualizar la energía de la oveja
   set monto-pasto monto-pasto - energia-del-pasto ;;  Descontar la energía del pasto consumido
   dibujar-pasto;; actualizar el color correspondiente del pasto
    ;; Este procedimiento se puede llamar desde el agente al patch dado que la oveja (agente) esta sobre el patch( pasto)
  ]
end
