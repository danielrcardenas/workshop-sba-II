globals [
  ...
  quemados% ;;porcentaje de arboles quemados
]
...
to setup
  ...
  ask patches with [(random-float 100) < densidad]
    [ set pcolor green ]
...
to go
  ...
  ask fires
    [ ask neighbors4 with [pcolor = green]
      ...
      set breed embers ]
  fade-embers
  set quemados% (burned-trees / initial-trees)
  tick
end
...
<experiments>
  <experiment name="experiment2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>burned%</metric>
    <steppedValueSet variable="density" first="50" step="2" last="60"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
