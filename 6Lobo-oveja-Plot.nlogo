...
to setup
 ...
 ask patches[
   set monto-pasto random-float 10.0
   dibujar-pasto
 ]
  create-ovejas n-ovejas [  ;; crear las ovejas
    setxy random-xcor random-ycor
    set color white
    set shape "sheep"
    set energia 100
  ]
   create-lobos n-lobos [  ;; crear los lobos
    setxy random-xcor random-ycor
    set color brown
    set shape "wolf"
    set energia 100
...
to go
  if not any? turtles [;; validar que hay agentes vivos para continuar la simulación
    stop
  ]

  ask ovejas [
    girar  ;; seleccionar una direccion de movimiento
    mover    ;; avanzar un paso e invertir energia
    comprobar-si-muere ;; si no tiene suficiente energia, validar si muere
    comer-pasto ;; tomar recursos del medio
    reproducir ;; comprobar si se puede reproducir
  ]
  ask lobos [
    girar  ;; first turn a little bit in each direction
    mover    ;; then step forward
    comprobar-si-muere
    comer-oveja ;; Cazar ovejas
    reproducir
  ]
  crecer-pasto ;; actualizar el crecimiento del pasto
  tick
end
to crecer-pasto
  ask patches[
  set monto-pasto monto-pasto + razon-crecimiento-pasto ;; incrementar la cantidad de pasto en la parcela
  if monto-pasto > 10 [
    set monto-pasto 10 ;; limite de pasto
  ]
  dibujar-pasto
  ]
end
...
to comer-oveja
  if any? ovejas-here [
    let objetivo one-of ovejas-here
    ask objetivo[
      die
    ]
    set energia energia + energia-de-oveja
  ]
end

to reproducir
  if energia > 200 [ ;; si el lobo o la oveja tienen energía suficiente entonces se crea una replica
    set energia energia - 100 ;; actualizar el nivel actual de energía del padre
    hatch 1 [ set energia 100]  ;; crear un agente hijo y asignarle energía inicial
  ]
end

to dibujar-pasto
  set pcolor scale-color green monto-pasto 0 20
end

;; ovejas procedure, the ovejas changes its heading
to girar
...
to mover
  ...
  set energia energia - costo-movimiento
end

to comer-pasto
 if(monto-pasto >= energia-del-pasto)[
   set energia energia + energia-del-pasto
   set monto-pasto monto-pasto - energia-del-pasto
   dibujar-pasto
 ]
end

to comprobar-si-muere
  if energia < 0 [die]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"ovejas" 1.0 0 -10649926 true "" "plot count ovejas"
"lobos" 1.0 0 -5207188 true "" "plot count lobos * 10"
