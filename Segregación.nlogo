globals [
  porcentaje-similitud       ;; promedio de cuántas tortugas en el vecindario son del mismo color de la tortuga
  porcentaje-insatisfaccion   ;; porcentaje de tortugas instatisfechas
]

turtles-own [
  feliz?               ;; Es verdadero si hay un número suficiente de tortugas del mismo color dentro del vecindario
  similares-alrededor  ;; Almacena el número de tortugas con el mismo color dentro del vecindario
  total-alrededor      ;; Suma las dos variables anteriores
]


to setup
  clear-all
  ;; Crea el número de tortugas determinado por el slider en lugares aleatorios de la matriz
  ;; El valor máximo del slider es 2500, que es un poco menos del número total de patches
  ask n-of numero patches [
    sprout 1
  ]
  ;; ¿Por qué no crear las tortugas y asignarles un patch?
  ask turtles [
    ;; Hacer que aproximadamente la mitad tome el color rojo y las demás verde
...
to encontrar-otro-lugar
  rt random-float 360
  fd random-float 10
  if any? other turtles-here
    [ encontrar-otro-lugar ]  ;; Seguir hasta que se encuentre un lugar vacío
  setxy pxcor pycor           ;; mover al centro del patch
                              ;; ¿Cuál es el efecto de esta última órden?
end

to actualizar-tortugas
  ask turtles [
    ;; En las siguientes dos líneas se usa el nativo "neighbors" para preguntar por los ocho patches aledaños
    set similares-alrededor count (turtles-on neighbors)
      with [color = [color] of myself]
    ;; cuenta el número total de vecinos
    set total-alrededor count (turtles-on neighbors)
    ;; La tortuga es feliz si se cumple con el número mínimo de vecinos similares
    set feliz? similares-alrededor >= ( similitud-deseada * total-alrededor / 100 )
  ]
end

to actualizar-globales
  let vecinos-similares sum [similares-alrededor] of turtles
  let total-vecinos sum [total-alrededor] of turtles
  set porcentaje-similitud (vecinos-similares / total-vecinos) * 100
  set porcentaje-insatisfaccion (count turtles with [not feliz?]) / (count turtles) * 100
end
...
"" ""
PENS
"percent" 1.0 0 -2674135 true "" "plot porcentaje-similitud"
...
"" ""
PENS
"percent" 1.0 0 -10899396 true "" "plot porcentaje-insatisfaccion"
