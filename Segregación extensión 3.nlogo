...
turtles-own [
  feliz?
  similares-alrededor
  otros-alrededor       ;; Número de vecinos con otro color
  total-alrededor
  mi-similitud-deseada
]
...
to actualizar-tortugas
  ask turtles [
    ...
    ;; cuenta el número de vecinos con color diferente
    set otros-alrededor count (turtles-on neighbors)
      with [color != [color] of myself]
    set total-alrededor count (turtles-on neighbors)
    set feliz? similares-alrededor >= ( mi-similitud-deseada * total-alrededor / 100 )
                  and otros-alrededor >= ( diferencia-deseada * total-alrededor / 100 )
    ;; La satisfacción depende de cumplir con el número mínimo de similitud y diferencia
  ]
end
