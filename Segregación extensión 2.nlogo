...
turtles-own [
  ...
  similares-alrededor
  total-alrededor
  mi-similitud-deseada  ;; El umbral para una tortuga específica
]


to setup
  clear-all
  set colores [ red green yellow blue orange ]
  ask n-of numero patches [
    sprout 1
  ]
  ask turtles [
    set color (item (random numero-de-grupos) colores)
    ;; Se le da un valor aleatorio a la similitud deseada entre 0 y el porcentaje del slider
    ;; ¿Alguna otra alternativa?
    set mi-similitud-deseada random similitud-deseada
  ]
  actualizar-tortugas
  actualizar-globales
  reset-ticks
...
to actualizar-tortugas
  ask turtles [
    ...
    ;; La tortuga es feliz si se cumple con el número mínimo de vecinos similares localmente
    ;; Es importante que se escoga "mi-similitud-deseada", no "similitud-deseada". ¿Por qué?
    set feliz? similares-alrededor >= ( mi-similitud-deseada * total-alrededor / 100 )
  ]
end
