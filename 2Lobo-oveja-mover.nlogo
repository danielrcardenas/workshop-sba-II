breed [ovejas una-oveja]
breed [lobos lobo]
patches-own [monto-pasto]
;; Este procedimiento inicia la configuración de la simulación
to setup
  clear-all


  create-ovejas n-ovejas [  ;; crea el numero inicial de ovejas
    setxy random-xcor random-ycor ;; ubicación aleatoria en el espacio
    set color white ;; las ovejas tendrán color blanco
    set shape "sheep" ;; Asignar la figura de la oveja al agente
  ]
   create-lobos n-lobos [  ;; Crear un número determinado de lobos
    setxy random-xcor random-ycor ;; ubicar aleatoriamente los lobos en el espacio
    set color brown ;; color cafe para los lobos
    set shape "wolf" ;; forma de los agentes tipo lobo
  ]
  crecer-pasto ;; llamar al procedimiento de crecer-pasto para dar una cantidad inicial
  reset-ticks ;; iniciar contador de pasos a cero
end

to go
  ask ovejas [
    girar  ;; girar un poco de manera aleatoria en cualquier dirección
    mover    ;; dar un paso en la dirección asignada
  ]
  ask lobos [
    girar  ;; girar en dirección aleatoria
    mover    ;; dar un paso hacia adelante
  ]
  tick
end
to crecer-pasto
  ask patches[
  set monto-pasto monto-pasto + razon-crecimiento-pasto
  if monto-pasto > 10 [
    set monto-pasto 10
  ]
  dibujar-pasto
...
;; Procedimienot para cambiar de dirección
to girar
  ;; Girar a la derecha o izquierda, en promedio sigue en dirección hacia el frente
  right random 90
  left random 90
end

;; Procedimiento para dar un paso hacia adelate en dirección del agente
to mover
  forward 1
end
@#$#@#$#@
