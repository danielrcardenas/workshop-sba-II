patches-own [vecinos-vivos]

to setup
  clear-all
  ask patches [

    set pcolor blue - 3 ;; asignar color para las celdas muertas
    if random 100 < 10 [ ;; aproximadamente un 10% de las celdas están vivas
      set pcolor green ;; Las celdas de color verde están vivas
    ]
  ]
  reset-ticks ;; Reiniciar contador de pasos
end

to go
  ask patches [
    ;; cada celda cuenta el número de vecinos vivos
    ;; y almacena el valor en la variable vecinos-vivos
    set vecinos-vivos count neighbors with [ pcolor = green ];; contar vecinos de color verde
  ]
  ask patches [

    ;; Primerca relgla: Si una celda está "viva" y tiene menos de dos
    ;; celdas vivas en su vecindad entonces muere
    if vecinos-vivos = 0 or vecinos-vivos = 1  [ set pcolor blue - 3 ]
    ;; Si una celda viva tiene más de tres vecinos vivos, entonces muere
    if vecinos-vivos >= 4 [ set pcolor blue - 3 ]

    ;; Si una celda NO viva tiene tres vecinos vivos, entonces vive
    if vecinos-vivos = 3 [ set pcolor green ]

    ;; En otro caso la celda se mantiene como está.
  ]
  tick
end
